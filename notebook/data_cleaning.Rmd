...
def apply_kmeans(df):
    ...
    visualizer = KElbowVisualizer(KMeans(), k=(2,15), metric='distortion', timings=False).fit(X)
...
for n_clusters in range_n_clusters:
    ...
    kmeans = KMeans(n_clusters=n_clusters, init='k-means++', n_init=15).fit(X)
...
def apply_agglomerative(df):
    ...
    davies = []
    for n in range(2, 10):
        agglo = AgglomerativeClustering(n_clusters=n, linkage='ward').fit(X)
#     df['Cluster_Agglo'] = agglo.labels_
        # Get the silhouette score to measure and compare the performance
        silhouette.append(metrics.silhouette_score(X, agglo.labels_, metric='euclidean'))
        calinski.append(metrics.calinski_harabasz_score(X, agglo.labels_))
        davies.append(metrics.davies_bouldin_score(X, agglo.labels_))
    
    plt.plot(range(2, 10), silhouette, 'r')
    plt.plot(range(2, 10), calinski, 'b')
    plt.plot(range(2, 10), davies, 'g')
    
    return df, silhouette

df_env, silhouette_env['AgglomerativeClustering'] = apply_agglomerative(df_env)
```

```{python}
df_env['Cluster_Agglo'].value_counts()
```
