...
```{python}
# df_env.drop(columns=['PostSecondary'], inplace=True)
# Data scaling
...
```{python}
# Run the kmeans model for each dataframe
inertias_kmeans = {}
silhouettes_kmeans = {}
calinskis_kmeans = {}
for data in datas:
    kmeans = KMeans(n_clusters=datas[data][2], init='k-means++', n_init=20).fit(datas[data][1])
    datas[data][0]['Cluster_kmeans'] = kmeans.labels_
    # Metrics
    inertias_kmeans[data] = kmeans.inertia_
    silhouettes_kmeans[data] = metrics.silhouette_score(datas[data][1], kmeans.labels_, metric='euclidean')
    calinskis_kmeans[data] = metrics.calinski_harabasz_score(datas[data][1], kmeans.labels_)
```
...
```{python}
df_env['Cluster_kmeans'].value_counts()
```
